...
class SpecsSAS(QWidget):
    ...
    def compare(self):
        ...
        try:
            ...
            for sheet in specsFile.keys():
                self.specsFile[sheet.lower()] = specsFile[sheet].applymap(lambda x:x.strip() if isinstance(x,str) else x)
    ...
    def visitCheck(self):
        ...
        self.missingTests = pd.DataFrame(columns=["Subject","Visit","Timepoint","Test Code"])
        possibleCombinations = self.sasData.assign(foo=1)[['VISIT',prefixVar,subjVar]].sort_values(subjVar).drop_duplicates().assign(foo=1).merge(self.specsFile['param'][[prefix+"TESTCD"]].assign(foo=1)).drop('foo',1).drop_duplicates()
...
if __name__ == '__main__':
    ...
    app.setStyleSheet("QPushButton#browseButton {background-color:white;border:none}"
